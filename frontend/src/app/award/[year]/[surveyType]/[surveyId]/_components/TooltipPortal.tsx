...
const getTextClasses = (type: 'bonus' | 'max-votes') => {
  const baseClasses =
    'flex flex-col font-["Pretendard",_sans-serif] font-normal justify-center text-[#000000]';
  const sizeClasses =
    type === 'bonus' ? 'text-[11px] md:text-base' : 'text-xs md:text-base';
  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        {config.showHideButton && onHide && <HideButton onHide={onHide} />}
      </motion.div>
    </motion.div>,
    document.body
  );
