...
export default function HomeBanner({ homeBannerDtos, className = "" }: HomeBannerProps) {
  ...
  return (
    <div 
      ref={bannerRef}
      className={`w-[320px] xs:w-[500px] sm:w-[500px] md:w-[550px] lg:w-[750px] max-w-[750px] h-auto md:h-[215px] relative bg-white rounded-xl outline outline-1 outline-offset-[-1px] outline-[#D1D1D6] overflow-hidden ${className}`}
    ...
    >
      ...
      <div className="hidden md:flex absolute bottom-4 right-4 gap-3 z-20">
        <button
          onClick={goToPrevious}
          className="w-8 h-8 items-center justify-center bg-black/20 hover:bg-black/40 rounded-full transition-all duration-200 flex"
          aria-label="이전 배너"
        >
          <img
