...
const getNormalStampBackground = (
  showGenderSelection: boolean,
  currentVotes: number,
  maxVotes: number
): string => {
...
const VoteCountText = ({
  ...
  const textColor = getNormalStampColor(
    showGenderSelection,
    currentVotes,
    maxVotes,
    isActive
    ...
    return (
      <div className="flex items-center gap-1.5 sm:gap-2 lg:gap-[13px]">
        <div className="flex items-center justify-center pt-1.5 pb-0.5 sm:pt-2 sm:pb-1 lg:pt-2.5 lg:pb-[5px]">
          <span
            className={`justify-center text-right ${TEXT_SIZES.large} leading-none font-bold ${textColor}`}
          >
            ×
          </span>
        </div>
        ...
        <div className="flex items-center justify-center pt-1.5 pb-0.5 sm:pt-2 sm:pb-1 lg:pt-2.5 lg:pb-[5px]">
          <span
            className={`justify-center text-right leading-none font-bold whitespace-nowrap ${textColor} ${textSizeClass}`}
          >
            {currentVotes}표
          </span>
        </div>
  ...
  return (
    <>
      <div className="flex w-12 items-center justify-center pt-1.5 pb-0.5 sm:w-16 sm:pt-2 sm:pb-1 lg:w-20 lg:pt-2.5 lg:pb-[5px]">
        <span
          className={`justify-center text-right leading-none font-bold whitespace-nowrap ${textColor} ${textSizeClass}`}
        >
          {currentVotes}표
      ...
      <div className="flex w-1.5 flex-col items-center justify-center self-stretch pt-1.5 sm:w-2 sm:pt-2 lg:pt-3">
        <span className="h-5 translate-y-0.5 justify-center self-stretch text-right text-sm leading-none font-bold text-black sm:h-7 sm:translate-y-1 sm:text-lg lg:h-9 lg:translate-y-2 lg:text-xl">
          /
        </span>
      </div>

      <div className="flex flex-col items-center justify-center pt-1.5 sm:pt-2 lg:pt-3">
        <span className="justify-center text-right text-sm leading-none font-bold text-black sm:text-lg lg:text-xl">
          {maxVotes}표
        </span>
      </div>
    </>
...
const BonusVoteCountText = ({
  ...
  if (showResult) {
    return (
      <div className="flex items-center gap-1.5 sm:gap-2 lg:gap-[13px]">
        <div className="flex items-center justify-center pt-1.5 pb-0.5 sm:pt-2 sm:pb-1 lg:pt-2.5 lg:pb-[5px]">
          <span
            className={`font-['Pretendard',_sans-serif] font-bold ${TEXT_SIZES.large} ${COLORS.bonus}`}
          >
            ×
          </span>
        </div>
        ...
        <div className="flex items-center justify-center pt-1.5 pb-0.5 sm:pt-2 sm:pb-1 lg:pt-2.5 lg:pb-[5px]">
          <span
            className={`font-['Pretendard',_sans-serif] font-bold ${COLORS.bonus} whitespace-nowrap ${textSizeClass}`}
          >
            {bonusVotesUsed}표
          </span>
        </div>
  ...
  return (
    <>
      ...
      <div className="flex w-1.5 flex-col items-center justify-center self-stretch pt-1.5 sm:w-2 sm:pt-2 lg:pt-3">
        <span
          className={`h-5 justify-center self-stretch text-right sm:h-7 lg:h-9 ${COLORS.bonus} translate-y-0.5 text-sm leading-none font-bold sm:translate-y-1 sm:text-lg lg:translate-y-2 lg:text-xl`}
        >
          /
        </span>
      </div>
      ...
      <div className="flex flex-col items-center justify-center pt-1.5 sm:pt-2 lg:pt-3">
        <div className="relative h-4 w-4 overflow-hidden sm:h-5 sm:w-5 lg:h-6 lg:w-6">
          <img
...
const VoteStamp = forwardRef<HTMLDivElement, VoteStampProps>(
  (
    ...
    if (type === 'normal') {
      ...
      const backgroundClass = getNormalStampBackground(
        showGenderSelection,
        currentVotes,
        maxVotes
      );
      const imageSrc = getNormalStampImage(
        showGenderSelection,
        currentVotes,
        maxVotes,
        weekDto
      ...
      return (
        <div className="flex items-center gap-3 sm:gap-4 lg:gap-[20px]">
          {/* Normal Vote Stamp */}
          <div
            className={`flex items-center justify-center ${stampSizes.mobile} ${stampSizes.tablet} ${stampSizes.desktop} ${roundedCorners} ${backgroundClass}`}
          >
            <img
              src={imageSrc}
              alt="Normal Vote Stamp"
              className={`h-full w-full object-cover ${roundedCorners}`}
            />
          </div>
