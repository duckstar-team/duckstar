...
export default function EpisodeItem({ 
    ...
    return (
      <div className="w-20 flex flex-col justify-start items-start gap-3 overflow-visible">
        ...
        <div className={`w-20 inline-flex justify-start items-center overflow-visible ${disableCursor ? '' : 'cursor-pointer'}`} onClick={onClick} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
          ...
          {!isLast && <div className="w-12 h-0 outline outline-1 outline-offset-[-0.58px] cursor-pointer pointer-events-none" style={{ outlineColor: '#FFB310' }}></div>}
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="w-20 flex flex-col justify-start items-start gap-3 overflow-visible">
        ...
        <div className={`w-20 inline-flex justify-start items-center overflow-visible ${disableCursor ? '' : 'cursor-pointer'}`} onClick={onClick} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
          ...
          {!isLast && <div className="w-12 h-0 outline outline-1 outline-offset-[-0.58px] pointer-events-none" style={{ outlineColor: '#CED4DA' }}></div>}
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="w-20 flex flex-col justify-start items-start gap-3 overflow-visible">
        ...
        <div className="w-20 inline-flex justify-start items-center overflow-visible cursor-pointer" onClick={onClick} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
          ...
          {!isLast && <div className="w-12 h-0 outline outline-1 outline-offset-[-0.58px] outline-rose-800 cursor-pointer pointer-events-none"></div>}
        </div>
      </div>
    );
  }
  ...
  // 기본값 (past)
  return (
    <div className="w-20 flex flex-col justify-start items-start gap-3 pb-[12px]">
      <div className="w-20 h-6 pl-2.5" />
      <div className="w-20 inline-flex justify-start items-center">
        <div className="w-8 h-8 rounded-2xl inline-flex flex-col justify-center items-center" style={{ backgroundColor: '#FFB310' }}>
          <div className="text-center text-white text-lg font-normal font-['Pretendard'] leading-tight">{episodeNumber}</div>
        </div>
        {!isLast && <div className="w-12 h-0 outline outline-1 outline-offset-[-0.58px] pointer-events-none" style={{ outlineColor: '#FFB310' }}></div>}
      </div>
    </div>
  );
}
