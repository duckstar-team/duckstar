...
export default function ThinNavDetail({ weeks = [], selectedWeek }: ThinNavDetailProps) {
  ...
  const generateMenuItems = () => {
    ...
    // 년도별로 그룹화
    const groupedByYear = weeks.reduce((acc, week) => {
      if (!acc[week.year]) {
        acc[week.year] = [];
      }
    ...
    Object.entries(groupedByYear).forEach(([year, yearWeeks]) => {
      ...
      // 분기별로 그룹화
      const groupedByQuarter = yearWeeks.reduce((acc, week) => {
        if (!acc[week.quarter]) {
          acc[week.quarter] = [];
        }
      ...
      Object.entries(groupedByQuarter).forEach(([quarter, quarterWeeks]) => {
        ...
        if (isExpanded) {
          quarterWeeks
            ...
            .forEach((week) => {
              const isWeekSelected = selectedWeek?.week === week.week && selectedWeek?.quarter === week.quarter;
              menuItems.push({
                type: "week" as const,
                label: `${week.week}주차`,
                state: isWeekSelected ? "selected" as const : "default" as const,
                weekData: week,
              });
            });
        }
          ...
          <ThinNavMenuItem
            ...
            onClick={() => {
              if (item.type === "week" && item.weekData) {
                // 주차 클릭 시 동적 라우팅으로 이동
                router.push(`/chart/${item.weekData.year}/${item.weekData.quarter}/${item.weekData.week}`);
                scrollToTop();
